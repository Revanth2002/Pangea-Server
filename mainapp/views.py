...
from pangeaserver.mail import sending_mail
...
class LoginUser(APIView):
    ...
    def post(self, request, fromat=None):
        ...
        get_otp_instance = UserOTPModel.objects.get_or_create(pid=user_instance.pid)[
            0]
...
class EnterLoginPin(APIView):
    ...
    def post(self, request, fromat=None):
        ...
        get_otp_instance = UserOTPModel.objects.filter(
            pid=user_instance.pid).first()
...
class PeopleTranscations(APIView):
    ...
    def get(self, request, fromat=None):
        ...
        transactions = TransactionModel.objects.filter(
            Q(from_pid=pid) | Q(to_pid=pid)).order_by('-created_at')
        ...
        for t in serializer.data:
            ...
            if(t['from_pid'] == pid):
                own_pid = False  # Sent
                user_instance = UserModel.objects.filter(
                    pid=t['to_pid']).first()
            else:
                user_instance = UserModel.objects.filter(
                    pid=t['from_pid']).first()
...
class HomeScreen(APIView):
    ...
    def get(self, request, format=None):
        ...
        cards = AddedCardsModel.objects.filter(
            Q(pid=pid)).order_by("-created_at")
        print("--------------------")
        print(len(cards))
        if len(cards) != 0:
        ...
        people = TransactionModel.objects.filter(
            from_pid=pid).values("to_pid").distinct()
        for person in people:
            person_instance = UserModel.objects.filter(
                pid=person["to_pid"]).first()
...
class SendPayment(APIView):
    ...
    def post(self, request, format=None):
        ...
        receiver_card_instance = AddedCardsModel.objects.filter(
            pid=to_pid).first()
        ...
        try:
            ...
            if redact_res["status"] == "success":
                ...
                try:
                    """Send a email to the user"""
                    mail_res = sending_mail(
                        request, "ZeroPay Transaction Pin Setup", txt_temp, user_instance.email)
                    print(f"Mail sent to {mail_res}")
...
class UpdateCard(APIView):
    ...
    def post(self, request):
        ...
        get_card = AddedCardsModel.objects.filter(
...
class Notifications(APIView):
    ...
    def convertdateformat(self, req_date):
        ...
        notifcation_date = dtt(x.year, x.month, x.day,
...
class SearchAPI(APIView):
    ...
    def get(self, request, *args):
        ...
        users = UserModel.objects.filter(Q(name__icontains=search) | Q(
            mobile__icontains=search)).exclude(pid=user.pid)
        serializers = UserSerializer(
            users, many=True, context={"request": request})
        for i in serializers.data:
            # get the user card details
            card = AddedCardsModel.objects.filter(
                pid=i["pid"], is_verified=True).first()
            if card is not None:
...
class CheckBalance(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        if pid in ["", None]:
            return display_response(
                msg="FAIL",
                err="Please provide pid",
                body=None,
                statuscode=status.HTTP_400_BAD_REQUEST
            )
    
        
        card = AddedCardsModel.objects.filter(pid=pid).first()
        if card is None:
            return display_response(
                msg="FAIL",
                err="Card not found",
        ...
        card_format = f"{card.card_number[:4]}-XXXX-XXXX-{card.card_number[12:16]}"
...
class VerifyCard(APIView):
    ...
    def get(self, request, **kwargs):
        params = request.query_params
        card_id = params.get("card_id", None)
        
        if card_id in ["", None]:
            return display_response(
                msg="FAIL",
                err="Please provide card id",
                body=None,
                statuscode=status.HTTP_400_BAD_REQUEST
            )
    
        card = AddedCardsModel.objects.filter(id=card_id).first()
        if card is None:
            return display_response(
                msg="FAIL",
                err="Card not found",
        ...
        get_user = UserModel.objects.filter(pid=card.pid).first()
        if get_user is None:
            return display_response(
                msg="FAIL",
                err="User not found",
        ...
        try:
            context_txt = f"Your card has been verified successfully. You can now use your card for payments."
            """Send a email to the user"""
            mail_res = sending_mail(
                request, "Card verified successfully", context_txt, get_user.email)
            print(f"Mail sent to {mail_res}")
